...
//    }
     ...
     * 计算文章评分，范围：0 - 0.6
     *
     * 文章评分 = 阅读量评分[0.1-0.7] - 时间评分[0-0.1]
     */
//    public static double calc(NewArticleCollection article) {
//        if (article == null || article.getReadCount() == null || article.getReadCount() == 0 || article.getCreateTime() == null || article.getPublishTime() == null) {
//            return 0d;
//        }
//
//        // 阅读量：0-1000w 转为 0-0.7
//        double rCountScore = log(10, article.getReadCount() + 1) / 10;
//        // 对于0-0.2的部分，压缩为0.1-0.2。避免被时间减成负数
//        if (rCountScore <= 0.2) {
//            rCountScore = rCountScore / 2 + 0.1;
//        }
//
//        // [抓取时间 - 发布时间]的小时数
//        int hours = (int) ((article.getCreateTime() - article.getPublishTime()) / 1000 / 3600);
//        // 时间：0-1024小时（约40天） 转为 0-0.1
//        double timeScore = log(2, hours + 1) / 100;
//
//        double score = rCountScore - timeScore;
//
//        // 上下限
//        if (score < 0) // 时间过长（40天以上）
//            score = 0;
//        if (score > 0.6) // 阅读量过大（1000w以上）
//            score = 0.6;
//        return score;
//    }
